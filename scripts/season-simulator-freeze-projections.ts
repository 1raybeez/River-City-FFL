...
const WEEK_2_KICKOFF = Date.parse("2026-09-17T20:15:00-04:00");
...
export function validateFreezeWindow(options: Pick<FreezeOptions, "season" | "week" | "now">): boolean {
  return options.season === 2026 && options.week === 2 && options.now.getTime() < WEEK_2_KICKOFF;
}
...
export async function runFreeze(options: FreezeOptions): Promise<FreezeSummary> {
  if (!validateFreezeWindow(options)) throw new Error("REFUSED: capture is after the approved Week 2 first kickoff or has invalid season/week.");
  ...
  const existing = (await fs.readdir(options.targetDirectory).catch(() => [] as string[])).filter(file => file.startsWith(`${options.season}-week-${String(options.week).padStart(2, "0")}-projection-evidence-`) && file.endsWith(".json"));
  if (existing.length > 0) throw new Error("REFUSED: an immutable Week 2 projection artifact already exists.");
  ...
  const summary: FreezeSummary = { season: options.season, week: options.week, source: "FANTASYPROS", sourceAsOf: feed.diagnostics.sourceAsOf, normalizedProjectionCount: feed.projections.length, franchisesFound: rosters.length, expectedScoresAvailable: Object.values(franchiseResults).filter(result => result.status === "AVAILABLE").length, unresolvedIdentities: feed.diagnostics.unresolved, unavailableTeams, unavailableReasons: Object.fromEntries(Object.values(franchiseResults).filter(result => result.status === "UNAVAILABLE").map(result => [result.franchiseId, result.reason])), checksumCandidate: artifact.checksum, artifactTargetPath: targetPath, writePerformed: false, beforeFirstKickoff: true };
  if (!options.dryRun) {
