...
interface SeasonPreviewSummary {
...
interface WorkbookParseResult {
  preview: AuctionImportPreview | null;
  summary: SeasonPreviewSummary;
}
...
function getPreviewRowSample(row: AuctionImportRow) {
  const raw = row.raw as MasterviewRawPayload;
  ...
  return {
    ...
    nflTeam: row.nflTeam,
    siteValues: raw.siteValues,
    lowValue: raw.lowValue,
    highValue: raw.highValue,
    averageValue: raw.averageValue,
    statusColumns: raw.statusColumns,
  };
}
...
function parseWorkbook(workbookPath: string): WorkbookParseResult {
    ...
    const averageValue =
      ...
      (rangeValues.length > 0
        ? rangeValues.reduce((total, value) => total + value, 0) /
          rangeValues.length
        : null);
    const statusColumns = getStatusColumns(original);
  ...
  return {
    ...
    summary: {
      ...
      columnsDetected,
      siteValueColumns,
      statusColumnsDetected: getStatusColumnsDetected(headerCells),
      rowsParsed: preview.rowCount,
      skippedRows,
      missingPlayerNames,
      missingPositions,
      warningCount: preview.warningCount,
      errorCount: preview.errorCount,
      sampleRows: preview.rows.slice(0, SAMPLE_ROW_LIMIT).map(getPreviewRowSample),
    },
  };
}

const workbookPaths = listWorkbookPaths();
const parseResults = workbookPaths.map(parseWorkbook);
  ...
  JSON.stringify(
    {
      ...
      rowsParsedTotal: summaries.reduce(
        (total, summary) => total + summary.rowsParsed,
        0
      ),
      skippedRowsTotal: summaries.reduce(
        (total, summary) => total + summary.skippedRows,
        0
      ),
      missingPlayerNamesTotal: summaries.reduce(
        (total, summary) => total + summary.missingPlayerNames,
        0
      ),
      missingPositionsTotal: summaries.reduce(
