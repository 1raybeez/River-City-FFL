...
async function main() {
  ...
  assert.equal(presentation.owedCount, 7);
  ...
  assert.equal(presentation.leagueDuesCents, 5_000);
  assert.equal(presentation.ownerCount, 12);
  assert.deepEqual(presentation.fundLocationSummary, ["Venmo", "VACU reserve/escrow", "PayPal when applicable"]);
  ...
  assert.equal(JSON.stringify(presentation).includes("paymentHandle"), false);
  assert.equal(JSON.stringify(presentation).includes("paymentMethod"), false);
  ...
  assert.equal("duesRows" in presentation, false);
  assert.equal(JSON.stringify(presentation).includes("Stan Schoppe"), false);
  ...
  assert.match(client, /2026 Season/);
  assert.match(client, /Where league funds are held/);
  assert.doesNotMatch(client, /payment handle|transaction id|commissioner note/i);
...
assert.match(client, /Historical archive/);
  ...
  assert.equal(approved.approvedAwards[0].recipient, "Ray Long");
  assert.equal(approved.approvedAwards[0].status, "APPROVED");
