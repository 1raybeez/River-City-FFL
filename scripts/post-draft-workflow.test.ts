...
assert.match(workflow, /postDraftReportFranchiseId/);
assert.match(workflow, /postDraftSourceFranchiseId/);
...
assert.match(workflow, /factualNarrativeCandidates/);
assert.match(workflow, /current\.strengths\.length > 0/);
assert.match(workflow, /teamAnalysis: buildPostDraftTeamAnalysis/);
assert.doesNotMatch(workflow, /candidates\.privateStrategyTake|candidates\.trashTalk|candidates\.internalNotes/);
...
assert.match(route, /generate-factual-draft/);
...
assert.match(client, /Generate factual candidates/);
assert.match(client, /Capture a locked post-draft snapshot and select a franchise/);
