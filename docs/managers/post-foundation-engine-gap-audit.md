...
| Records | `lib/history/ownerCareerSummary.ts`; `lib/history/ownerMatchupSummary.ts`; `app/league-info/archives/page.tsx`; `app/history/page.tsx`; `lib/stats.ts`; `lib/manual-history.ts` | Approved career accessors; legacy `calculateAllTimeStats()` | Approved placement 2011–2025 and matchup facts 2018–2026; Archives independently fetches 2018–2026 roster settings | Archives and `/history` | Partial/duplicated. Owner placement and matchup records already exist, but league record-book categories and lineage do not. Two pages calculate or read legacy facts independently. |
...
`app/history/page.tsx` calls `calculateAllTimeStats()` in `lib/stats.ts`, which
derives championships, podiums, average finish, and best/worst finish from the
unused manual ledger. This duplicates approved season and career results.
...
| `app/history/page.tsx` and `lib/stats.ts` | Titles, podiums, average/best/worst finish from `MANUAL_HISTORY` | Historical Season Results / Owner Career Summary |
...
| `/history` | `calculateAllTimeStats()` over `MANUAL_HISTORY` | Legacy duplicate to replace with approved result/record sources |
