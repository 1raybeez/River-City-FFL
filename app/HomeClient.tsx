...
function formatCountdown(state: BoxOneState, now: Date | null) {
  const target = state.state === "DRAFT_UPCOMING" ? state.draftStartAt : state.seasonStartAt;
  if (!target || !now) return null;
  return getCountdownParts(now, target);
}

export default function HomeClient({ initialMember, initialPublishedRecap, initialBoxOneState }: { initialMember: CurrentMember; initialPublishedRecap: PublicLeagueRecap | null; initialBoxOneState: BoxOneState }) {
  ...
  const seasonDateTime = formatEasternDateTime(boxOneState.seasonStartAt);
  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-slate-950 dark:bg-[#0a0a0a] dark:text-white">
    ...
    <main className="mx-auto grid max-w-7xl gap-5 px-4 pb-12 sm:px-6 md:grid-cols-2 lg:grid-cols-12 lg:px-8" aria-label="Home dashboard">
      <section className="contents" aria-label="Primary season status">
        ...
        {boxOneState.state !== "DRAFT_UPCOMING" && <DashboardCard label="2026 League Event" icon={<CalendarDays size={17} className="text-orange-600" />}><h2 className="mt-5 text-2xl font-black uppercase italic leading-none">{boxOneState.title}</h2>{boxOneState.state === "DRAFT_LIVE" && <><p className="mt-4 text-sm font-semibold">Drafting now</p><p className="mt-1 text-sm text-slate-500 dark:text-white/55">{draftDateTime?.time ?? "Live draft"} · Location TBD</p></>}{boxOneState.state === "POST_DRAFT_PRESEASON" && <><p className="mt-4 text-sm font-semibold">{seasonDateTime?.date ?? "Season start date unavailable"}</p><p className="mt-1 text-sm text-slate-500 dark:text-white/55">{seasonDateTime?.time ?? "Season start time unavailable"}</p><div className="mt-6"><MiniStat label="Season countdown" value={boxOneCountdown ? `${boxOneCountdown.days} DAYS · ${boxOneCountdown.hours} HRS · ${boxOneCountdown.minutes} MIN` : "Unavailable"} /></div><Link href="/matchups" className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-lg bg-blue-700 px-5 py-3 text-[10px] font-black uppercase tracking-widest text-white">View Matchups <ArrowRight size={14} /></Link></>}{boxOneState.state === "SEASON_LIVE" && <><p className="mt-4 text-sm font-semibold">River City 2026 is underway.</p><Link href="/matchups" className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-lg bg-blue-700 px-5 py-3 text-[10px] font-black uppercase tracking-widest text-white">View Matchups <ArrowRight size={14} /></Link></>}{boxOneState.state === "DATA_UNAVAILABLE" && <p className="mt-4 text-sm text-slate-500 dark:text-white/55">Draft and season details are temporarily unavailable.</p>}</DashboardCard>}
      </section>
