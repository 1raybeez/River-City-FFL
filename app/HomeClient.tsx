...
import { getNflCardEyebrow, getNflLiveDetail, resolveNflWeek, type NflGameCenterState } from "@/lib/nflGameCenter";
  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-slate-950 dark:bg-[#0a0a0a] dark:text-white">
    ...
    <main className="mx-auto grid max-w-7xl gap-5 px-4 pb-12 sm:px-6 md:grid-cols-2 lg:grid-cols-12 lg:px-8" aria-label="Home dashboard">
      ...
      <section className="contents" aria-label="League activity and finance">
        <DashboardCard label="Predictor" icon={<TrendingUp size={17} className="text-blue-600" />}><h2 className="mt-5 text-2xl font-black uppercase italic leading-none">What Are My Chances?</h2><p className="mt-4 text-sm leading-6 text-slate-600 dark:text-white/60">{predictorProgress?.readiness === "PRODUCTION_READY" ? "Approved production outlook is available." : predictorProgress?.readiness === "SHADOW_READY" ? "Probability model validating. Approved projected standings remain separate from private shadow probabilities." : "Season projections are being calibrated with real projection-vs-actual evidence."}</p><div className="mt-5 grid grid-cols-2 gap-2 text-xs"><MiniStat label="Projection Baseline" value={predictorProgress?.projectionBaselines.find(row => row.week === 2)?.status === "CAPTURED" ? "Week 2 · Captured" : "Waiting"} /><MiniStat label="Projected Standings" value={predictorProgress?.projectedStandingsStatus === "READY" ? "Available" : "Waiting for future-week projection data"} /><MiniStat label="Player Samples" value={`${predictorProgress?.playerSamples ?? 0} / 50`} /><MiniStat label="Team Samples" value={`${predictorProgress?.teamSamples ?? 0} / 12`} /><MiniStat label="Eligible Weeks" value={String(predictorProgress?.eligibleWeeks.length ?? 0)} /></div><p className="mt-4 rounded-lg bg-slate-100 p-3 text-[10px] leading-4 text-slate-500 dark:bg-white/5 dark:text-white/55"><strong>Next:</strong> {predictorProgress?.nextEvent ?? "Loading calibration status..."}<br /><strong>Status:</strong> {predictorProgress?.readiness ?? "CALIBRATING"}<br /><strong>Odds:</strong> {predictorProgress?.readiness === "SHADOW_READY" ? "Probability model validating" : predictorProgress?.readiness === "PRODUCTION_READY" ? "Approved" : "Calibrating"}</p><Link href="/predictor" className="mt-5 inline-flex min-h-10 items-center gap-2 text-[10px] font-black uppercase tracking-widest text-blue-700">Open Predictor <ArrowRight size={14} /></Link></DashboardCard>
