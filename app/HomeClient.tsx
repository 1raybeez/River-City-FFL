...
export default function HomeClient({ initialMember, initialPublishedRecap, initialBoxOneState, initialLiveSeasonState }: { initialMember: CurrentMember; initialPublishedRecap: PublicLeagueRecap | null; initialBoxOneState: BoxOneState; initialLiveSeasonState: HomeLiveSeasonState }) {
  ...
  const showDraftRecap = liveSeasonState.finality.finalizedWeek === null;
  const liveSeasonSpotlightLabel = liveSeasonState.weeklyHighScore.length > 0
    ? "WEEKLY HIGH SCORE"
    : liveSeasonState.phase === "CHAMPIONSHIP_WEEK"
      ? "CHAMPIONSHIP SPOTLIGHT"
      : liveSeasonState.phase === "PLAYOFFS"
        ? "PLAYOFF SPOTLIGHT"
        : "2026 WEEKLY SPOTLIGHT";
