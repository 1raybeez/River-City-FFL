...
export default function HomeClient({ initialMember }: { initialMember: CurrentMember }) {
  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-slate-950 dark:bg-[#0a0a0a] dark:text-white">
    <nav className="sticky top-0 z-50 border-b border-white/10 bg-[#071a33]/95 px-4 py-3 text-white backdrop-blur-md sm:px-6"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4"><Link href="/" className="flex min-w-0 items-center gap-3" aria-label="River City FFL home"><span className="relative h-11 w-11 shrink-0 overflow-hidden rounded-full border border-white/20 bg-white"><Image src="/River City FFL Logo.JPG" alt="" fill className="object-cover" unoptimized /></span><span className="hidden min-w-0 sm:block"><span className="block text-lg font-black uppercase italic leading-none">River City FFL</span><span className="mt-1 block text-[8px] font-bold uppercase tracking-[0.18em] text-white/55">A tradition of competition</span></span></Link><div className="hidden min-w-0 items-center gap-1 lg:flex">{[["Home", "/"], ["Matchups", "/matchups"], ["Managers", "/managers"], ["Rivalries", "/league-info/rivalries"], ["History", "/history"], ["League Info", "/league-info"]].map(([label, href]) => <Link key={href} href={href} aria-current={href === "/" ? "page" : undefined} className={`rounded-md px-3 py-2 text-[10px] font-black uppercase transition ${href === "/" ? "border-b-2 border-amber-400 text-white" : "text-white/65 hover:bg-white/10 hover:text-white"}`}>{label}</Link>)}{initialMember.authenticated ? <div className="ml-3 flex min-w-0 items-center gap-2 border-l border-white/20 pl-3" aria-label="Member account"><div className="min-w-0 text-right"><p className="truncate text-[10px] font-black uppercase">{initialMember.displayName ?? "League member"}</p>{initialMember.franchiseName && <p className="truncate text-[9px] text-white/60">{initialMember.franchiseName}</p>}</div>{initialMember.canAccessWarRoom && <Link href="/commish/auction" className="rounded-md border border-white/25 px-2 py-2 text-[9px] font-black uppercase hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400">My War Room</Link>}{initialMember.canAccessMaintenance && <Link href="/commish" className="rounded-md border border-amber-300/60 px-2 py-2 text-[9px] font-black uppercase text-amber-200 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400">Commissioner Hub</Link>}<SignOutControl /></div> : <Link href="/member/login?returnTo=%2F" className="ml-3 rounded-md border border-white/35 px-3 py-2 text-[10px] font-black uppercase text-white transition hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400">League Member Login</Link>}</div><button type="button" aria-label={isMobileMenuOpen ? "Close navigation menu" : "Open navigation menu"} aria-expanded={isMobileMenuOpen} aria-controls="home-mobile-navigation" onClick={() => setIsMobileMenuOpen((open) => !open)} className="rounded-lg border border-white/25 p-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 lg:hidden">{isMobileMenuOpen ? <X size={18} /> : <Menu size={18} />}</button></div>{isMobileMenuOpen && <div id="home-mobile-navigation" className="mx-auto mt-3 grid max-w-7xl grid-cols-2 gap-2 rounded-xl border border-white/15 bg-[#0b2444] p-3 lg:hidden">{visibleMobileNavLinks.map(([label, href]) => <Link key={href} href={href} onClick={() => setIsMobileMenuOpen(false)} className="rounded-lg border border-white/10 px-3 py-3 text-[9px] font-black uppercase tracking-widest text-white/75 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400">{label}</Link>)}{initialMember.authenticated ? <div className="col-span-2 border-t border-white/15 pt-3" aria-label="Member account"><p className="text-xs font-black uppercase">{initialMember.displayName ?? "League member"}</p>{initialMember.franchiseName && <p className="mt-1 text-[10px] text-white/60">{initialMember.franchiseName}</p>}<div className="mt-3 flex flex-wrap gap-2">{initialMember.canAccessWarRoom && <Link href="/commish/auction" onClick={() => setIsMobileMenuOpen(false)} className="min-h-10 rounded-lg border border-white/25 px-3 py-3 text-[9px] font-black uppercase focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400">My War Room</Link>}{initialMember.canAccessMaintenance && <Link href="/commish" onClick={() => setIsMobileMenuOpen(false)} className="min-h-10 rounded-lg border border-amber-300/60 px-3 py-3 text-[9px] font-black uppercase text-amber-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400">Commissioner Hub</Link>}<SignOutControl className="min-h-10" /></div></div> : <Link href="/member/login?returnTo=%2F" onClick={() => setIsMobileMenuOpen(false)} className="col-span-2 min-h-11 rounded-lg border border-amber-300/60 px-3 py-3 text-center text-[10px] font-black uppercase tracking-widest text-amber-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400">League Member Login</Link>}</div>}</nav>
