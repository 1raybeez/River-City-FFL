...
import { recordCommissionerAwardSettlement } from "@/lib/finance/operationalFinanceAwardSettlement";
...
export async function POST(
  ...
  const { season: seasonValue, obligationId } = await context.params;
  const season = Number(seasonValue);
  if (season !== 2026) {
    return NextResponse.json({ error: "Award payments currently support 2026 only." }, { status: 404 });
  }
  try {
    const result = await recordCommissionerAwardSettlement(
    ...
    const message = error instanceof Error ? error.message : "Award payment could not be recorded.";
    const status = /Idempotency|exceed|fully paid/.test(message) ? 409 : 400;
