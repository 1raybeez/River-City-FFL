...
export async function POST(req: Request, context: { params: Promise<{ season: string }> }) {
  ...
  if (season !== 2026) return NextResponse.json({ error: "Expense creation currently supports 2026 only." }, { status: 404 });
  try {
    const mutation = await createOperationalFinanceExpense(
    ...
    return NextResponse.json({ error: message }, { status: /already exists|Idempotency/.test(message) ? 409 : 400 });
  }
}
