...
import { recordOperationalFinanceContribution } from "@/lib/finance/operationalFinanceExpenses";
...
export async function POST(req: Request, context: { params: Promise<{ season: string }> }) {
  ...
  if (season !== 2026) return NextResponse.json({ error: "Contribution recording currently supports 2026 only." }, { status: 404 });
  try {
    const mutation = await recordOperationalFinanceContribution(
      getOperationalFinanceLedgerRepository(season), season, await req.json(), authorization.actor, new Date().toISOString()
    );
    ...
    const message = error instanceof Error ? error.message : "Contribution recording failed.";
    return NextResponse.json({ error: message }, { status: /Idempotency/.test(message) ? 409 : 400 });
  }
}
