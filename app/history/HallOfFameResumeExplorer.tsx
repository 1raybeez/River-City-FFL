      ...
      <div className="mt-5 rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="hidden md:block">
          <table className="w-full text-left text-sm">
            ...
            <tbody>{visibleRankings.map((stat) => <tr key={stat.ownerId} className={`border-t border-slate-100 ${stat.rank <= 3 ? "bg-amber-50/50" : ""}`}><td className="px-5 py-4"><span className={`inline-flex min-w-8 items-center justify-center rounded-full px-2 py-1 font-black ${stat.rank <= 3 ? "bg-amber-400 text-slate-950" : "bg-slate-100 text-slate-600"}`}>{stat.rank}</span></td><th scope="row" className="px-5 py-4 font-black text-slate-950"><div className="flex items-center gap-3"><OwnerPortrait name={stat.manager} photo={ownerProfilesById[stat.ownerId]?.photo} className="h-10 w-10" /><div><span aria-label={stat.coOwnerLabel ? `${stat.manager} with ${stat.coOwnerLabel}` : stat.manager}>{stat.manager}</span>{stat.coOwnerLabel && <span className="mt-1 block text-xs font-semibold text-slate-500">with {stat.coOwnerLabel}</span>}</div></div></th><td className="px-5 py-4 text-center text-lg font-black text-orange-700">{stat.championships}</td><td className="px-5 py-4 text-center font-bold text-slate-800">{stat.podiumFinishes}</td><td className="px-5 py-4 text-center font-semibold text-slate-700">{stat.averageFinish.toFixed(2)}</td><td className="px-5 py-4 text-center text-slate-700">{stat.seasonsPlayed}</td><td className="px-5 py-4 text-right"><span className="text-xs font-black tracking-widest text-slate-500">{stat.status}</span></td></tr>)}</tbody>
          </table>
        </div>
        <div className="grid gap-3 p-4 md:hidden">{visibleRankings.map((stat) => <article key={stat.ownerId} className={`rounded-xl border p-4 ${stat.rank <= 3 ? "border-amber-300 bg-amber-50/60" : "border-slate-200 bg-slate-50"}`} aria-labelledby={`manager-${stat.ownerId}`}><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><OwnerPortrait name={stat.manager} photo={ownerProfilesById[stat.ownerId]?.photo} className="h-10 w-10" /><h3 id={`manager-${stat.ownerId}`} className="break-words font-black text-slate-950"><span className={`mr-2 inline-flex min-w-7 items-center justify-center rounded-full px-2 py-1 text-xs ${stat.rank <= 3 ? "bg-amber-400 text-slate-950" : "bg-slate-200"}`}>#{stat.rank}</span>{stat.manager}</h3></div><span className="shrink-0 text-[10px] font-black tracking-widest text-slate-500">{stat.status}</span></div>{stat.coOwnerLabel && <p className="mt-1 text-xs font-semibold text-slate-500" aria-label={`Co-owner ${stat.coOwnerLabel}`}>with {stat.coOwnerLabel}</p>}<ResumeMetrics stat={stat} /></article>)}</div>
      </div>
