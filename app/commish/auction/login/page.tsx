  ...
  const signInWithGoogle = async () => {
    ...
    try {
      ...
      if (!response.ok || !payload.success) {
        throw new Error(
          payload.error ??
            'That Google account is not allowed to access Auction War Room.'
        );
      }
        ...
        <section className="rounded-3xl border border-black/10 bg-white p-6 shadow-xl dark:border-white/10 dark:bg-[#121212] sm:p-8">
          <div className="mb-8 flex flex-col gap-5 sm:flex-row sm:items-center">
            ...
            <div>
              ...
              <h1 className="text-4xl font-black uppercase italic leading-none tracking-tighter sm:text-5xl">
                River City Auction War Room
              </h1>
            </div>
          </div>
          ...
          <button
            ...
            className="inline-flex w-full items-center justify-center gap-3 rounded-2xl bg-orange-600 px-5 py-4 text-xs font-black uppercase tracking-widest text-white shadow-lg transition hover:bg-orange-700 disabled:cursor-not-allowed disabled:opacity-60"
          >
