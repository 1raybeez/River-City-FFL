...
function ResultCard({ result }: { result: ServerRecommendationDiagnostic["teamRecommendations"][number] }) {
  ...
  return <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#121212]">
    ...
    <div className="mt-4 grid gap-4 text-sm md:grid-cols-3">
      ...
      <div><h4 className="font-black uppercase tracking-widest">FantasyCalc REDRAFT</h4>{result.tradeMarket.direction === "UNAVAILABLE" ? <p className="mt-1">FantasyCalc REDRAFT: unavailable</p> : <><p className="mt-1">{result.tradeMarket.direction} · Δ {value(result.tradeMarket.difference)}</p><p className="mt-1">Outgoing {value(result.tradeMarket.outgoingValue)} · incoming {value(result.tradeMarket.incomingValue)}</p></>}</div>
...
function Coverage({ diagnostic }: { diagnostic: ServerRecommendationDiagnostic }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#121212]"><h2 className="text-xl font-black uppercase italic">Server context coverage · {overall}</h2><ul className="mt-3 grid gap-2 text-sm font-bold md:grid-cols-2"><Status ok={coverage.sleeperLeague}>Sleeper league</Status><Status ok={coverage.rosters}>Rosters</Status><Status ok={coverage.ownership}>Authoritative ownership</Status><Status ok={coverage.expertRos.available && coverage.expertRos.validation === "PASS"}>Expert ROS: {coverage.expertRos.playerCount} players · {coverage.expertRos.runtime}</Status><Status ok={coverage.fantasyCalcRedraft.available}>FantasyCalc REDRAFT: {coverage.fantasyCalcRedraft.playerCount} players · 1 QB / 12 teams / 0.5 PPR / isDynasty=false</Status><Status ok={coverage.acquisitionSnapshot}>Acquisition snapshot</Status><Status ok={coverage.keeperHistory}>Keeper history</Status><Status ok={coverage.fairnessEngine}>Fairness engine</Status><Status ok={coverage.seasonMode !== "UNAVAILABLE"}>Season mode: {coverage.seasonMode}</Status></ul><p className="mt-4 text-xs font-bold leading-5">ROS artifact: {coverage.expertRos.artifactId} · validation {coverage.expertRos.validation} · generated {coverage.expertRos.generatedAt ?? "N/A"} · sources {coverage.expertRos.sourceNames.join(", ") || "N/A"} · checksum {coverage.expertRos.checksum ?? "N/A"}</p></section>;
}
