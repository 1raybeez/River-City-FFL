...
export type WeeklyOperationsSnapshot = {
  ...
  health: { headline: string; lifecycle: string; actionRequired: boolean; attentionOwner: "ENGINEERING" | "COMMISSIONER" | null; durableEvidence: string; scheduler: string; schedulerStatus: WeeklyOperationRunHealth["status"]; lastSuccessfulRunAt: string | null; lastFailedRunAt: string | null; failureReason: string | null; affectedSystems: readonly string[] };
...
export async function buildWeeklyOperationsSnapshot(now = new Date()): Promise<WeeklyOperationsSnapshot> {
  ...
  const schedulerLabel = schedulerHealth.status === "ERROR"
    ? `${SETTLEMENT_FUNCTION_NAME} configured · runtime failure detected · engineering attention required · last failed ${schedulerHealth.lastFailedRunAt ?? "unknown"}`
  ...
  const engineeringAttention = schedulerHealth.status === "ERROR";
  return {
    ...
    health: { headline: schedulerHealth.status === "ERROR" ? "SCHEDULER NEEDS ENGINEERING ATTENTION" : schedulerHealth.status === "UNKNOWN" ? "SCHEDULER STATUS UNKNOWN" : "ALL SYSTEMS GREEN", lifecycle, actionRequired: false, attentionOwner: engineeringAttention ? "ENGINEERING" : null, durableEvidence: "DESIGN READY · PRODUCTION STORAGE NOT YET ACTIVATED", scheduler: schedulerLabel, schedulerStatus: schedulerHealth.status, lastSuccessfulRunAt: schedulerHealth.lastSuccessfulRunAt, lastFailedRunAt: schedulerHealth.lastFailedRunAt, failureReason: schedulerHealth.failureReason, affectedSystems: schedulerHealth.affectedSystems },
    issues, checklist: [
