...
export async function buildWeeklyOperationsSnapshot(now = new Date()): Promise<WeeklyOperationsSnapshot> {
  ...
  const [currentRows, settlements, publishedRecaps, artifact] = await Promise.all([
    getMatchups(currentWeekValue, LEAGUE_ID),
    listWeeklyHighScoreSettlements(OPERATIONS_SEASON), listPublishedWeeklyRecaps(OPERATIONS_SEASON), actualArtifact(1),
  ]);
  ...
  return {
    ...
    freeze: { week: 2, status: freezeDecision.eligible ? "READY TO FREEZE" : freezeDecision.reason === "BEFORE_APPROVED_FREEZE_WINDOW" ? "NOT OPEN" : "CLOSED / NOT ELIGIBLE", statusLevel: freezeDecision.eligible ? "BLUE" : "YELLOW", windowOpen: freezeWindow?.windowOpen ?? null, firstKickoff: freezeWindow?.firstKickoff ?? null, command: PROJECTION_FREEZE_COMMAND, message: freezeDecision.eligible ? "Window is open, but this control plane never runs the freeze." : "No authoritative Week 2 baseline currently exists; the Sep 11 diagnostic remains non-authoritative." },
    predictor: { status: "CALIBRATING", baselineWeek1: false, actualWeek1: Boolean(artifact), residualWeek1: false, baselineWeek2: false, actualWeek2: false, message: "Week 1 PATH C actuals are historical-only. Week 2 probabilities remain gated until valid frozen evidence exists." },
    issues, checklist: [
      ...
      { status: "YELLOW", label: "Week 2 projection freeze", detail: "No authoritative baseline exists; execute only in the approved window after all checks pass.", command: PROJECTION_FREEZE_COMMAND },
